...
function patients()
{
	...
	while ($row = mysqli_fetch_array($query)) {
		...
		?>
		<td><center><a href="deletepatient.php?id=<?php echo $row['id'] ?>" onclick="return confirm('Are you sure to delete this record?')" ><img src='../assets/img/glyphicons-17-bin.png' height='16px' width='12px'></a></center></td>
...
function addpatient()
{
	...
	$sql = "INSERT INTO patient VALUES ('','$ddate','$fname','$sname','$email','$address','$phone','$gender','$birthyear')";
	$query = mysqli_query($con,$sql);
	if (!empty($query)) {
...
{	global $con;
	...
	if ($doctor=="WomenDoctor") {
		$price = 40000;
		 $ddate=date('Y-m-d');
				...
				$sql = "INSERT INTO medication VALUES ('','$ddate','$id','WomenDoctor','','','','','$doctor','$price','','','$day','$month','$year')";
	...
	elseif ($doctor=="NormalDoctor") {
		$price = 20000;
		 $ddate=date('Y-m-d');
		$sql = "INSERT INTO medication VALUES ('','$ddate','$id','recdoctor','','','','','$doctor','$price','','','$day','$month','$year')";
	...
	elseif ($doctor=="DentalDoctor") {
		...
		$sql = "INSERT INTO medication VALUES ('','$ddate','$id','DentalDoctor','','','','','$doctor','$price','','','$day','$month','$year')";
    	...
    	elseif ($doctor=="Admission") {
		$price = 30000;
		 $ddate=date('Y-m-d');

		$sql = "INSERT INTO medication VALUES ('','$ddate','$id','Admission','','','','','$doctor','$price','','','$day','$month','$year')";

			$query = mysqli_query($con,$sql);
			if (!empty($query)) {
				echo "<br><b style='color:#008080;font-size:14px;font-family:Arial;'>Patient is Succesifully Sent to Admission</b><br><br>";
			}
			else{
				echo mysqli_error();
			}
      ...
      elseif ($doctor=="EyeClinic") {
		$price = 30000;
		 $ddate=date('Y-m-d');

		$sql = "INSERT INTO medication VALUES ('','$ddate','$id','EyeClinic','','','','','$doctor','$price','','','$day','$month','$year')";

			$query = mysqli_query($con,$sql);
			if (!empty($query)) {
				echo "<br><b style='color:#008080;font-size:14px;font-family:Arial;'>Patient is Succesifully Assigned To Eye Clinic Succesifully</b><br><br>";
			}
			else{
				echo mysqli_error();
			}
    ...
    elseif ($doctor=="Pharmacy") {
		$price = 30000;
        $ddate=date('Y-m-d');
		$sql = "INSERT INTO medication VALUES ('','$ddate','$id','Pharmacy','','','','','$doctor','$price','','','$day','$month','$year')";

			$query = mysqli_query($con,$sql);
			if (!empty($query)) {
				echo "<br><b style='color:#008080;font-size:14px;font-family:Arial;'>Patient is Succesifully Sent To Pharmacy</b><br><br>";
			}
			else{
				//echo mysqli_error();
			}
   ...
   elseif ($doctor=="Laboratory") {
		$price = 30000;
		 $ddate=date('Y-m-d');

		$sql = "INSERT INTO medication VALUES ('','$ddate','$id','Laboratory','','','','','$doctor','$price','','','$day','$month','$year')";

			$query = mysqli_query($con,$sql);
			if (!empty($query)) {
				echo "<br><b style='color:#008080;font-size:14px;font-family:Arial;'>Patient is Succesifully sent to Laboratory</b><br><br>";
			}
			else{
				echo mysqli_error();
			}
